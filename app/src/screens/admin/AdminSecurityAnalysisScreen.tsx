...
export default function AdminSecurityAnalysisScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get('/security-analysis/overview');
        if (a) setData(res.data);
      } catch {
        if (a) setData(null);
      } finally {
  ...
  return (
    <ScrollView style={styles.box} contentContainerStyle={{ padding: 16 }}>
      <Text style={styles.h}>Security overview</Text>
