...
const PAYMENT_METHODS = [
  { id: 'card',         labelKey: 'checkout.paymentMethods.card',  icon: '💳' },
  { id: 'paypal',       labelKey: 'checkout.paymentMethods.paypal',               icon: '🅿️' },
  { id: 'mobile_money', labelKey: 'checkout.paymentMethods.mobileMoney',         icon: '📱' },
  { id: 'crypto',       labelKey: 'checkout.paymentMethods.crypto',       icon: '₿' },
];
  ...
  if (items.length === 0) return (
    <BuyerLayout>
      <div className="flex flex-col items-center justify-center h-[70vh] gap-4">
        ...
        <h2 className="text-lg font-bold" style={{ color: '#1a1a1a' }}>{t('checkout.emptyCart')}</h2>
        <Link to="/">
          <button className="px-6 py-2.5 rounded-2xl text-white font-semibold"
            style={{ background: 'linear-gradient(135deg,#ff8c42,#ff5f00)' }}>{t('checkout.shopNow')}</button>
        </Link>
      </div>
    </BuyerLayout>
  );
      ...
      <div className="w-full px-4 sm:px-6 lg:px-10 xl:px-16 py-8">
            ...
            <div key={s.id} className="flex items-center flex-1">
              <button onClick={() => s.id < step && setStep(s.id)}
                ...
                <span className="text-xs font-semibold hidden sm:block"
                  style={{ color: step >= s.id ? '#ff8c42' : '#9ca3af' }}>{t(s.key)}</span>
              </button>
              {i < STEPS.length - 1 && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                {step === 1 && (
                  <div className="space-y-4">
                    <h2 className="text-lg font-black mb-4" style={{ color: '#1a1a1a' }}>📍 {t('checkout.shippingAddress')}</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {[['fullName','checkout.fields.fullName'],['email','checkout.fields.email'],['phone','checkout.fields.phone'],['street','checkout.fields.street'],['city','checkout.fields.city'],['state','checkout.fields.state'],['zip','checkout.fields.zip'],['country','checkout.fields.country']].map(([f, ph]) => (
                        <input key={f} type="text" placeholder={t(ph)} value={address[f]}
                ...
                {step === 2 && (
                  <div className="space-y-3">
                    <h2 className="text-lg font-black mb-4" style={{ color: '#1a1a1a' }}>🚚 {t('checkout.deliveryMethod')}</h2>
                      ...
                      <button key={opt.id} onClick={() => setDelivery(opt.id)}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            <p className="font-semibold text-sm" style={{ color: '#1a1a1a' }}>{t(opt.labelKey)}</p>
                            <p className="text-xs" style={{ color: '#9ca3af' }}>{t(opt.subKey)}</p>
                          </div>
                        </div>
                    ...
                    <div className="flex gap-3 pt-2">
                      <button onClick={prevStep} className="flex-1 py-3 rounded-2xl font-semibold text-sm"
                        style={{ background: '#f3f4f6', color: '#374151' }}>{t('common.back')}</button>
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                ...
                {step === 3 && (
                  <div className="space-y-4">
                    <h2 className="text-lg font-black mb-4" style={{ color: '#1a1a1a' }}>💳 {t('checkout.paymentMethod')}</h2>
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      {PAYMENT_METHODS.map(pm => (
                        <button key={pm.id} onClick={() => setPayment(pm.id)}
                          ...
                          <span className="text-xs font-semibold" style={{ color: payment === pm.id ? '#ff8c42' : '#374151' }}>{t(pm.labelKey)}</span>
                        </button>
                      ))}
                    </div>
                    {payment === 'card' && (
                      <div className="space-y-3 p-4 rounded-2xl" style={{ background: '#fafafa', border: '1px solid #f3f4f6' }}>
                        <input placeholder={t('checkout.fields.cardholderName')} value={cardInfo.name}
                          onChange={e => setCardInfo({ ...cardInfo, name: e.target.value })} className={inp} style={inpStyle} />
                        <input placeholder={t('checkout.fields.cardNumber')} value={cardInfo.number}
                          onChange={e => setCardInfo({ ...cardInfo, number: e.target.value })} className={inp} style={inpStyle} />
                        <div className="grid grid-cols-2 gap-3">
                          <input placeholder={t('checkout.fields.expiry')} value={cardInfo.expiry}
                            onChange={e => setCardInfo({ ...cardInfo, expiry: e.target.value })} className={inp} style={inpStyle} />
                          <input placeholder={t('checkout.fields.cvv')} value={cardInfo.cvv}
                ...
                {step === 4 && (
                  <div className="space-y-4">
                    <h2 className="text-lg font-black mb-4" style={{ color: '#1a1a1a' }}>✅ {t('checkout.reviewYourOrder')}</h2>
                      ...
                      {items.map(item => (
                        <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl"
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs" style={{ color: '#9ca3af' }}>{t('checkout.qty')} {item.quantity}</p>
                          </div>
                    ...
                    <label className="flex items-start gap-2 cursor-pointer">
                      ...
                      <span className="text-xs" style={{ color: '#6b7280' }}>
                        {t('checkout.agreePrefix')} <span style={{ color: '#ff8c42' }}>{t('checkout.termsOfService')}</span> {t('checkout.agreeMiddle')} <span style={{ color: '#ff8c42' }}>{t('checkout.refundPolicy')}</span>
                      </span>
                    </label>
                    <div className="flex items-center gap-2 p-3 rounded-xl" style={{ background: 'rgba(34,197,94,0.07)', border: '1px solid rgba(34,197,94,0.2)' }}>
                      ...
                      <span className="text-xs" style={{ color: '#16a34a' }}>{t('checkout.escrowNote')}</span>
                    </div>
                    <div className="flex gap-3">
                      ...
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                        ...
                        style={{ background: 'linear-gradient(135deg,#ff8c42,#ff5f00)', boxShadow: '0 6px 20px rgba(255,140,66,0.35)' }}>
                        {placing ? t('checkout.placingOrder') : `${t('checkout.placeOrder')} · $${total.toFixed(2)}`}
                      </motion.button>
                    </div>
                  </div>
                )}
          ...
          <div className="rounded-2xl p-5 h-fit sticky top-20 space-y-4"
            ...
            <h3 className="font-black text-sm" style={{ color: '#1a1a1a' }}>{t('checkout.orderSummary')}</h3>
            ...
            <div className="border-t pt-3 space-y-2">
              {[[t('checkout.subtotal'), `$${subtotal.toFixed(2)}`], [t('checkout.shipping'), `$${shippingCost.toFixed(2)}`], [t('checkout.tax10'), `$${tax.toFixed(2)}`]].map(([l, v]) => (
            ...
            <div className="flex items-center gap-1.5 p-2 rounded-xl text-xs" style={{ background: '#f0fdf4', color: '#16a34a' }}>
              <Lock className="w-3.5 h-3.5 flex-shrink-0" /> {t('checkout.escrowProtected')}
            </div>
          </div>
        </div>
      </div>
