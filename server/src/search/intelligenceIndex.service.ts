...
export async function buildUserDocs(limit = BATCH): Promise<IntelligenceSearchDocument[]> {
  ...
  return rows.map((u) => {
    ...
    return {
      ...
      deepLink: isSeller ? `/admin/sellers?userId=${id}` : `/admin/users?userId=${id}`,
...
export async function buildOrderDocs(limit = BATCH): Promise<IntelligenceSearchDocument[]> {
  ...
  return rows.map((o) => {
    ...
    return {
      ...
      subtitle: `${o.customer || 'Customer'} · ${o.currency || 'USD'} ${Number(o.total || 0).toFixed(2)}`,
      status: o.status,
      module: 'orders',
      moduleLabel: 'Orders',
      deepLink: `/admin/orders?orderId=${id}`,
      searchText: [o.orderNumber, o.customer, o.status].filter(Boolean).join(' '),
      metadata: { orderNumber: String(o.orderNumber || '') },
...
export async function mongoIntelligenceSearch(
  ...
  if (want('order')) {
    tasks.push(
      (async () => {
        const rows = await Order.find({ orderNumber: regex })
          .select('orderNumber status total currency customer date createdAt')
          .limit(limitPerType)
          .lean();
...
function buildUserDocsFromRows(rows: any[]): IntelligenceSearchDocument[] {
  return rows.map((u) => {
    const id = String(u._id);
    const isSeller = u.role === 'seller';
    const type: IntelligenceEntityType = isSeller ? 'seller' : 'user';
...
function mapOrderDoc(o: any): IntelligenceSearchDocument {
  const id = String(o._id);
  return {
    id: docId('order', id),
    entityType: 'order',
    ...
    searchText: '',
    metadata: { orderNumber: String(o.orderNumber || '') },
    updatedAt: new Date(o.createdAt || Date.now()).getTime(),
  };
}

function mapPaymentDoc(t: any): IntelligenceSearchDocument {
  const id = String(t._id);
...
function mapProductDoc(p: any): IntelligenceSearchDocument {
  const id = String(p._id);
  return {
    id: docId('product', id),
    entityType: 'product',
...
function mapVehicleDoc(v: any): IntelligenceSearchDocument {
  const id = String(v._id);
  return {
    id: docId('vehicle', id),
    entityType: 'vehicle',
...
function mapSupportDoc(t: any): IntelligenceSearchDocument {
  const id = String(t._id);
  return {
    id: docId('support', id),
    entityType: 'support',
...
function mapSubscriptionDoc(s: any): IntelligenceSearchDocument {
